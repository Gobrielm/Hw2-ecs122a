...
\tab minPenalty $\leftarrow$ min(dp[i - j] + penalty, minPenalty)\\
\tab len += 1\\
...
\tab\tab	j = j + 1\\
\tab\tab	penalty = calculatePenalty(len)\\
\tab\tab	if i = n:\\
\tab\tab\tab break out if $len > 16$\\
\tab\tab\tab minPenalty $\leftarrow$ min(dp[i - j], minPenalty)\\
\tab\tab	else:\\
\tab\tab\tab minPenalty $\leftarrow$ min(dp[i - j] + penalty, minPenalty)\\
\tab\tab     len += 1\\

calculatePenalty(len) would look like this..\\

if $len > 16$: \\
\tab penalty $\leftarrow (len - 16)^2$\\
else:\\
\tab penalty $\leftarrow (16 - len)^2$\\
...
In each recurrence call, we consider filling a line with a number of words. We look at many cases where a line could be filled with one word up to 32 characters + extra characters from the last word. This covers all potential cases since any line with more than 32 characters, a potential word would incur a bigger penalty at the end of such a line rather than just being put on the next line.\\
